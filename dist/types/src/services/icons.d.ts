export declare enum FaIcons {
    $add = "fas fa-plus",
    $delete = "fas fa-trash-alt",
    $sliders = "fas fa-sliders-h",
    $edit = "fas fa-pen",
    $menu = "fas fa-bars",
    $chevronDown = "fas fa-chevron-down",
    $caretDown = "fas fa-caret-down",
    $radioOn = "far fa-dot-circle",
    $radioOff = "far fa-circle",
    $checkOn = "fas fa-check-square",
    $checkOff = "far fa-square",
    $arrowRight = "fas fa-chevron-right",
    $arrowLeft = "fas fa-chevron-left",
    $arrowUp = "fas fa-arrow-up",
    $filter = "fas fa-align-justify",
    $marker = "fas fa-map-marker-alt",
    $search = "fas fa-search",
    $close = "fas fa-times",
    $paperclip = "fas fa-paperclip"
}
export declare enum MaterialIcons {
    $add = "add",
    $delete = "delete_forever",
    $sliders = "fas fa-sliders-h",
    $edit = "fas fa-pen",
    $menu = "fas fa-bars",
    $expand = "expand_more",
    $group = "expand_more",
    $subgroup = "arrow_drop_down",
    $dropdown = "fas fa-caret-down",
    $radioOn = "far fa-dot-circle",
    $radioOff = "far fa-circle",
    $checkOn = "check_box",
    $checkOff = "check_box_outline_blank",
    $arrowRight = "chevron_right",
    $arrowLeft = "chevron_left",
    $arrowUp = "arrow_upward",
    $filter = "filter_list",
    $marker = "fas fa-map-marker-alt",
    $search = "search",
    $close = "clear"
}
export declare enum MaterialSizes {
    'xs' = 14,
    's' = 18,
    'md' = 20,
    'l' = 28,
    'xl' = 36
}
export declare enum FaSizes {
    'xs' = 12,
    's' = 14,
    'md' = 18,
    'l' = 24,
    'xl' = 32
}
