...
export const VDatePicker = defineComponent({
  ...
  setup(props, {
    ...
    const handlers = ref({});
    const closeConditional = ref(true);
    const inputSlotRef = ref(null);
    ...
    function genDatepickerDisplayInner() {
      ...
      return h('div', propsData, [genDisplayValue((_data$selected = data.selected) === null || _data$selected === void 0 ? void 0 : _data$selected.year), genDisplayValue(displayDate.value)]);
    }
    ...
    function genDatepickerHeader() {
      return h(VDatepickerHeader, {
        ...
        onMouseleave: () => closeConditional.value = !data.isYears && !data.isMonths
      }, {
