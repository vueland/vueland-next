...
import { h, ref, reactive, watch, computed, renderSlot, withDirectives, defineComponent, onMounted, vShow } from 'vue';
...
export const VTooltip = defineComponent({
  ...
  setup(props, {
    ...
    function genContent() {
      ...
      return withDirectives(h('span', setBackground(props.color, propsData), slots.default && slots.default()), [[vShow, innerActive.value]]);
    }
    ...
    watch(() => isActive.value, to => innerActive.value = to, {
      immediate: true
    });
    onMounted(() => {
    ...
    return () => {
      const content = useTransition(genContent(), innerActive.value ? 'scale-in' : 'fade');
      return [h('div', {
        class: classes.value
